    ...
    if (dataSource) {
        if (void 0 !== value) {
            ...
            if (oldValue !== value) {
                ...
                if ("pageSize" === optionName && 0 === value) {
...
class DataController extends((0, _m_data_helper_mixin.DataHelperMixin)(_m_modules.default.Controller)) {
    ...
    updateItems() {
        let change = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {};
        let isDataChanged = arguments.length > 1 ? arguments[1] : void 0;
    ...
    needUpdateDimensions(operationTypes) {
        return operationTypes && (operationTypes.reload || operationTypes.paging || operationTypes.groupExpanding)
    }
    ...
    getAllDataRowKeys() {
        return Promise.resolve(this.loadAll(void 0)).then(items => items.filter(item => "data" === item.rowType).map(item => item.key))
    }
