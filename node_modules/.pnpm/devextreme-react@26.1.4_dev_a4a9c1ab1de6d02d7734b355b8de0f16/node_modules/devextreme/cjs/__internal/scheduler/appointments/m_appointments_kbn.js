...
var _is_focus_lost = require("../utils/is_focus_lost");
...
class AppointmentsKeyboardNavigation {
    ...
    delHandler(e) {
        if (this._collection.option("allowDelete")) {
            e.preventDefault();
            const settings = this._collection.getAppointmentSettings((0, _renderer.default)(e.target));
    ...
    restoreFocusAfterDelete(sortedIndex) {
        if (!(0, _is_focus_lost.isFocusLost)()) {
            return
        }
    ...
    findNearestItem(sortedIndex) {
        const $items = this._collection.$itemBySortedIndex;
        for (let index = sortedIndex; index >= 0; index -= 1) {
            const $item = null === $items || void 0 === $items ? void 0 : $items[index];
            if (null !== $item && void 0 !== $item && $item.length) {
                return $item
            }
        }
        return
