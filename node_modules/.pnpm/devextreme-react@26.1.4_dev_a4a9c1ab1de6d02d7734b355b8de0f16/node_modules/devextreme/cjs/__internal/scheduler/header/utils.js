/**
 * DevExtreme (cjs/__internal/scheduler/header/utils.js)
 * Version: 26.1.4
...
var _skipped_days = require("../../scheduler/utils/skipped_days");
...
const getDateAfterWeek = (startDate, firstDayOfWeek, skippedDays) => {
    ...
    while ((0, _skipped_days.isDateSkipped)(lastVisibleDate, skippedDays)) {
...
const getDateAfterVisibleDays = (startDate, dayCount, skippedDays, direction) => {
    ...
    let date = (0, _skipped_days.getFirstVisibleDate)(new Date(startDate), skippedDays, dateStep);
    for (let i = 0; i < dayCount; i += 1) {
        date = (0, _skipped_days.getFirstVisibleDate)(dateStep(date), skippedDays, dateStep)
    }
    return date
};
const getIntervalStartDate = options => {
    ...
    switch (step) {
        case "day":
            return (0, _skipped_days.getFirstVisibleDate)(getPeriodStart(date, step, false, firstDayOfWeek), skippedDays, nextDay);
        case "month":
            return getPeriodStart(date, step, false, firstDayOfWeek);
        case "week": {
            const weekStart = getPeriodStart(date, "week", false, firstDayOfWeek);
            return (0, _skipped_days.getFirstVisibleDate)(weekStart, skippedDays, nextDay)
        }
        default:
...
const getNextPeriodStartDate = (currentPeriodEndDate, step, skippedDays) => {
    ...
    if ("day" === step || "week" === step) {
        date = (0, _skipped_days.getFirstVisibleDate)(date, skippedDays, nextDay)
    }
    return date
};
