/**
 * DevExtreme (esm/__internal/scheduler/appointments_new/resizing/get_resized_dates.js)
 * Version: 26.1.4
...
export const isStartDateResized = _ref => {
    let {
        handles: handles,
...
const correctEndDateByDelta = (endDate, deltaTime, startDayHour, endDayHour) => {
...
const correctStartDateByDelta = (startDate, deltaTime, startDayHour, endDayHour) => {
...
export const getResizedDates = options => {
    const {
        startDate: startDate,
        endDate: endDate,
        deltaTime: deltaTime,
        isStartDateChanged: isStartDateChanged,
        needCorrectDates: needCorrectDates,
        startDayHour: startDayHour,
        endDayHour: endDayHour
    } = options;
    if (isStartDateChanged) {
        const correctedStart = needCorrectDates ? correctStartDateByDelta(startDate, deltaTime, startDayHour, endDayHour) : startDate.getTime() - deltaTime;
        const startTime = correctedStart + timeZoneUtils.getTimezoneOffsetChangeInMs(startDate, endDate, new Date(correctedStart), endDate);
        return {
            startDate: new Date(startTime),
            endDate: new Date(endDate.getTime())
        }
    }
    const correctedEnd = needCorrectDates ? correctEndDateByDelta(endDate, deltaTime, startDayHour, endDayHour) : endDate.getTime() + deltaTime;
    const endTime = correctedEnd - timeZoneUtils.getTimezoneOffsetChangeInMs(startDate, endDate, startDate, new Date(correctedEnd));
    return {
