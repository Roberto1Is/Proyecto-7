...
class LayoutManager extends Widget {
    ...
    _getLocationCssClasses(location) {
        ...
        if (locationFlags.isFirstRow) {
            cssClasses.push("dx-first-row")
        }
        if (locationFlags.isFirstCol) {
            cssClasses.push("dx-first-col")
        }
        if (locationFlags.isLastCol) {
            cssClasses.push("dx-last-col")
        }
        if (locationFlags.isLastRow) {
            cssClasses.push("dx-last-row")
        }
        return cssClasses
    }
    ...
    _updateItemsCssClasses() {
        ...
        null === responsiveBoxItems || void 0 === responsiveBoxItems || responsiveBoxItems.forEach(responsiveBoxItem => {
            ...
            $itemContainer.toggleClass("dx-first-col", isFirstCol).toggleClass("dx-last-col", isLastCol).toggleClass("dx-first-row", isFirstRow).toggleClass("dx-last-row", isLastRow);
            const element = $itemContainer.get(0);
            if (element) {
                element.className = [...element.classList].filter(name => !name.startsWith(FORM_FIELD_ITEM_COL_CLASS)).join(" ")
            }
