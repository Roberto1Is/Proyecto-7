...
class SchedulerAppointments extends _collection_widget.default {
    ...
    getAppointmentSettings($item) {
        return (0, _renderer.default)($item).data(_constants.APPOINTMENT_SETTINGS_KEY)
    }
    _moveFocus() {}
    _focusTarget() {
    ...
    _renderAppointmentTemplate($container, appointment, model) {
        ...
        if (!this.isAgendaView) {
            $container.parent().prepend((0, _renderer.default)("<div>").addClass(_classes.APPOINTMENT_CONTENT_CLASSES.STRIP))
        }
    }
    ...
    _createItemByTemplate(itemTemplate, renderArgs) {
        ...
        parent.prepend((0, _renderer.default)("<span>").addClass(_classes.APPOINTMENT_CONTENT_CLASSES.ARIA_DESCRIPTION).attr("hidden", true));
    ...
    renderAgendaAppointment(element, settings) {
        if (settings.isLastInGroup) {
            element.addClass(_classes.AGENDA_LAST_IN_DATE_APPOINTMENT_CLASS)
        }
        const {
            groups: groups,
    ...
    getDateRange(e, startDate, endDate) {
        ...
        if (isStartDateChanged) {
            ...
            startTime += _utils_time_zone.default.getTimezoneOffsetChangeInMs(startDate, endDate, startTime, endDate);
            endTime = endDate.getTime()
        } else {
            startTime = startDate.getTime();
            endTime = needCorrectDates ? this._correctEndDateByDelta(endDate, deltaTime) : endDate.getTime() + deltaTime;
            endTime -= _utils_time_zone.default.getTimezoneOffsetChangeInMs(startDate, endDate, startDate, endTime)
        }
        return {
    ...
    _removeDragSourceClassFromDraggedAppointment() {
        const $appointments = this._itemElements().filter(`.${_classes.APPOINTMENT_DRAG_SOURCE_CLASS}`);
