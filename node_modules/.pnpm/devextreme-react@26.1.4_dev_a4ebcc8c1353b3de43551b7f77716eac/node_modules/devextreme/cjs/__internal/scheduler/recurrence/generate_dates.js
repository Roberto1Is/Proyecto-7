...
const getRruleParams = options => {
    ...
    const clientOffsets_startDate = _utils_time_zone.default.getClientTimezoneOffset(start),
        clientOffsets_minViewDate = _utils_time_zone.default.getClientTimezoneOffset(min),
        clientOffsets_maxViewDate = _utils_time_zone.default.getClientTimezoneOffset(max);
    ...
    const startDateDSTDifferenceMs = _utils_time_zone.default.getDiffBetweenClientTimezoneOffsets(options.start, startIntervalDate);
...
const getLocalMachineOffset = rruleDate => {
    const machineTimezoneOffset = _utils_time_zone.default.getClientTimezoneOffset(rruleDate);
    const machineTimezoneName = _utils_time_zone.default.getMachineTimezoneName();
...
const convertRruleResult = (rruleIntervalParams, options, rruleDate) => {
    ...
    const convertedDateDSTShift = _utils_time_zone.default.getDiffBetweenClientTimezoneOffsets(convertedBackDate, rruleDate);
    const switchToSummerTime = convertedDateDSTShift < 0;
    const resultDate = addOffsets(convertedBackDate, convertedDateDSTShift);
    const resultDateDSTShift = _utils_time_zone.default.getDiffBetweenClientTimezoneOffsets(resultDate, convertedBackDate);
...
const createRRule = (options, startDateUtc, until) => {
    ...
    if (until) {
        ruleOptions.until = addOffsets(until, -_utils_time_zone.default.getClientTimezoneOffset(until), options.appointmentTimezoneOffset)
    }
    ...
    if (options.exception) {
        ...
        exceptionDates.forEach(date => {
            const rruleTimezoneOffsets = "function" === typeof options.getExceptionDateTimezoneOffsets ? options.getExceptionDateTimezoneOffsets(date) : [-_utils_time_zone.default.getClientTimezoneOffset(date), options.appointmentTimezoneOffset];
