...
class AreaItem {
    ...
    _createTableElement() {
        return $("<table>").attr("id", `dx-${new Guid}`).attr("role", "presentation")
    }
    ...
    _createFakeTable() {
        const that = this;
        if (!that._fakeTable) {
            that._fakeTable = that.tableElement().clone().removeAttr("id").attr("aria-hidden", "true").addClass(FAKE_TABLE_CLASS);
