...
export class TreeListDataController extends DataController {
    ...
    needUpdateDimensions(operationTypes) {
        return super.needUpdateDimensions(operationTypes) || operationTypes && operationTypes.nodeExpanding
    }
    changeRowExpand(key) {
        if (this._dataSource) {
            const args = {
    ...
    getAllDataRowKeys() {
        ...
        const keys = [];
        null === (_this$_dataSource = this._dataSource) || void 0 === _this$_dataSource || _this$_dataSource.forEachNode(node => {
            keys.push(node.key)
        });
