...
import {
    formatImplicitSchedulerDate,
    formatImplicitSchedulerTime
} from "../../utils/global_formats";
const localizeDate = date => formatImplicitSchedulerDate(date);
const localizeTime = date => formatImplicitSchedulerTime(date);
...
export const getAriaDescription = async options => {
    ...
    const text = [getGroupText(options), ...resources, options.allowDelete ? messageLocalization.format("dxScheduler-hotkeysAriaDescription-delete") : null, messageLocalization.format("dxScheduler-hotkeysAriaDescription-homeEnd")].filter(Boolean).join("; ");
    return text
};
