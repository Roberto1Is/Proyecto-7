...
class Splitter extends CollectionWidgetLiveUpdate {
    ...
    _resizeHandler() {
        ...
        const {
            _ignoreSizeConstraints: _ignoreSizeConstraints
        } = this.option();
    ...
    _setPanesCacheSize() {
        ...
        items.forEach((item, index) => {
            ...
            if (!isDefined(null === restriction || void 0 === restriction ? void 0 : restriction.size)) {
                return
            }
            if (true === item.collapsed) {
                const isLastNonCollapsed = index >= findLastIndexOfNonCollapsedItem(items);
                const isLastVisible = this._isLastVisibleItem(index);
                const direction = isLastVisible || isLastNonCollapsed ? CollapseExpandDirection.Previous : CollapseExpandDirection.Next;
            ...
            if (false === item.visible) {
                ...
                const direction = isLastVisible ? CollapseExpandDirection.Previous : CollapseExpandDirection.Next;
    ...
    _getResizeHandleConfig(paneId) {
        ...
        return {
            ...
            onResizeStart: e => {
                ...
                this._currentOnePxRatio = convertSizeToRatio(1, getElementSize($(this.element()), currentOrientation), this._getResizeHandlesSize());
    ...
    _recalculateLayout(delta, paneIndex) {
        this._itemRestrictions.forEach(pane => {
            pane.resizable = void 0
        });
        const currentLayout = this.getLayout();
        let newLayout = getNextLayout(currentLayout, delta, paneIndex, this._itemRestrictions);
        if (newLayout === currentLayout) {
            this._itemRestrictions.forEach(pane => {
                pane.maxSize = void 0
            });
            newLayout = getNextLayout(currentLayout, delta, paneIndex, this._itemRestrictions)
        }
        this._layout = newLayout;
        this._idealLayout = this._layout;
        this._applyStylesFromLayout(this.getLayout());
    ...
    _dimensionChanged() {
        ...
        if (!idealLayout || 0 === idealLayout.length) {
            return
        }
        const {
            orientation: orientation,
    ...
    _getEligiblePaneIndices(pixels, remaining) {
        ...
        for (let index = 0; index < pixels.length; index += 1) {
            const item = items[index];
            if (!item || false === item.visible || true === item.collapsed || false === item.resizable && isDefined(this._initialPaneSizes[index])) {
                continue
            }
            const minPx = tryConvertToNumber(item.minSize, elementSize) ?? 0;
            const maxPx = tryConvertToNumber(item.maxSize, elementSize);
            const clampedPx = this._getClampedPixelSize(pixels[index] + direction, minPx, maxPx);
            if (0 !== compareNumbersWithPrecision(clampedPx, pixels[index])) {
                indices.push(index)
            }
        }
        return indices
    ...
    _distributeRemainingPixels(pixels, initialRemaining) {
        ...
        while (0 !== compareNumbersWithPrecision(remaining, 0)) {
            const eligiblePaneIndices = this._getEligiblePaneIndices(pixels, remaining);
            if (0 === eligiblePaneIndices.length) {
    ...
    _applyPixelShare(pixels, eligiblePaneIndices, share) {
        ...
        eligiblePaneIndices.forEach(index => {
            ...
            const minPx = tryConvertToNumber(null === item || void 0 === item ? void 0 : item.minSize, elementSize) ?? 0;
            const maxPx = tryConvertToNumber(null === item || void 0 === item ? void 0 : item.maxSize, elementSize);
            const clampedPx = this._getClampedPixelSize(next, minPx, maxPx);
            const delta = clampedPx - prev;
            if (0 !== compareNumbersWithPrecision(delta, 0)) {
