...
class OptionManager {
    constructor(schedulerStore, layoutData) {
    ...
    getPanelOptions(panelName) {
        var _layoutData$getGroupH;
        const layoutData = this.layoutData ?? this.schedulerStore.getWorkSpace();
        const panelDOMSize = layoutData.getPanelDOMSize("vertical" === this.options.groupOrientation ? "regularPanel" : panelName);
        const groupHeights = null === (_layoutData$getGroupH = layoutData.getGroupHeights) || void 0 === _layoutData$getGroupH ? void 0 : _layoutData$getGroupH.call(layoutData, panelName);
        const cacheKey = [panelDOMSize.width, panelDOMSize.height, panelName, groupHeights];
