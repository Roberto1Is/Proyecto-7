...
var _drop_down_editor = require("../../ui/drop_down_editor/drop_down_editor");
...
var _text_editor = require("../../ui/text_box/text_editor.base");
...
class Form extends _widget.default {
    ...
    _getDefaultOptions() {
        return Object.assign({}, super._getDefaultOptions(), {
            ...
            labelMode: "outside",
            isDirty: false,
            onSmartPasting: void 0,
            onSmartPasted: void 0
        })
    }
    ...
    _getContent() {
        ...
        return scrollingEnabled ? (0, _renderer.default)(null === (_this$getScrollable = this.getScrollable()) || void 0 === _this$getScrollable ? void 0 : _this$getScrollable.content()) : this.$element()
    }
    ...
    _refresh() {
        const editorSelector = `.${_text_editor.TEXTEDITOR_CLASS}.${_widget.FOCUSED_STATE_CLASS}:not(.${_drop_down_editor.DROP_DOWN_EDITOR_CLASS}) .${_text_editor.TEXTEDITOR_INPUT_CLASS}`;
    ...
    _updateLayoutsOnDimensionChange() {
        this._cachedLayoutManagers.forEach(layoutManager => {
            layoutManager.updateResponsiveBoxLayout()
        });
        this._updateGroupsColCount();
        this._alignLabels(this._rootLayoutManager, this._rootLayoutManager.isSingleColumnMode())
    }
    _applyGroupColCount($group, colCount) {
        const oldColCount = $group.attr(_constants.GROUP_COL_COUNT_ATTR);
        if (oldColCount) {
            $group.removeClass(`${_constants.GROUP_COL_COUNT_CLASS}${oldColCount}`)
        }
        if (!this._groupsColCount.includes(colCount)) {
            this._groupsColCount.push(colCount)
        }
        $group.addClass(`${_constants.GROUP_COL_COUNT_CLASS}${colCount}`).attr(_constants.GROUP_COL_COUNT_ATTR, colCount)
    }
    _updateGroupsColCount() {
        this._groupsColCount = [];
        this._cachedLayoutManagers.forEach(layoutManager => {
            if (layoutManager === this._rootLayoutManager) {
                return
            }
            const $group = layoutManager.$element().closest(`.${_constants.FORM_GROUP_CLASS}`);
            if (!$group.length) {
                return
            }
    ...
    getScrollable() {
        ...
        return scrollingEnabled ? this._scrollable : void 0
    }
    updateDimensions() {
