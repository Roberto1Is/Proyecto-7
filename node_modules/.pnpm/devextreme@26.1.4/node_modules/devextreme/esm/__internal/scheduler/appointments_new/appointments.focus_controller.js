...
export class AppointmentsFocusController {
    ...
    constructor(appointments, handlers) {
    ...
    restoreFocusAfterDelete(sortedIndex) {
        if (!isFocusLost()) {
            return
        }
    ...
    findNearestViewItem(sortedIndex) {
    ...
    handleDeleteKeyDown(viewItem) {
        ...
        const appointmentViewItem = viewItem;
    ...
    handleEnterKeyDown(viewItem, e) {
        e.originalEvent.preventDefault();
        if (viewItem instanceof AppointmentCollector) {
            return
        }
        this.handlers.onAppointmentEnterKeyDown(viewItem, e.originalEvent)
    }
    focusBySortedItem(sortedItem) {
    ...
    scrollToItem(sortedItem) {
        const {
            getStartViewDate: getStartViewDate,
            getResourceManager: getResourceManager,
            scrollTo: scrollTo
        } = this.appointments.option();
        const date = new Date(Math.max(getStartViewDate().getTime(), sortedItem.source.startDate));
        const group = getRawAppointmentGroupValues(sortedItem.itemData, getResourceManager().resources);
        scrollTo(date, {
            group: group,
            allDay: sortedItem.allDay
        })
    }
}
