/**
 * DevExtreme (cjs/__internal/scheduler/appointments_new/resizing/get_appointment_date_range.js)
 * Version: 26.1.4
...
const getResizedDateRange = options => {
    const {
        rtlEnabled: rtlEnabled,
        handles: handles,
        appointment: appointment,
    ...
    if (isStartEdgeResized) {
        const firstCell = rtlEnabled ? getCellData(options, Math.floor(leftCell.index / cellCountInRow), leftCell.index - cellsAmount + 1, appointment.isOccupiedAllDay, appointment.isAllDay) : leftCell;
        return {
            startDate: firstCell.startDate,
            endDate: firstCell.startDate > appointment.endDate ? firstCell.startDate : appointment.endDate
        }
    }
    const lastCellIndex = leftCell.index + cellsAmount - 1;
    const lastCell = rtlEnabled ? leftCell : getCellData(options, Math.floor(lastCellIndex / cellCountInRow), lastCellIndex % cellCountInRow, appointment.isOccupiedAllDay, appointment.isAllDay);
    return {
        startDate: lastCell.endDate < appointment.startDate ? lastCell.endDate : appointment.startDate,
