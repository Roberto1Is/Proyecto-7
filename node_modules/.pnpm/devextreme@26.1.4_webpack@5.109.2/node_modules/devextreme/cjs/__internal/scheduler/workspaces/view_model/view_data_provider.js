/**
 * DevExtreme (cjs/__internal/scheduler/workspaces/view_model/view_data_provider.js)
 * Version: 26.1.4
...
class ViewDataProvider {
    ...
    update(options, isGenerateNewViewData) {
        ...
        const dateHeaderDataGenerator = new _date_header_data_generator.DateHeaderDataGenerator(viewDataGenerator);
        const timePanelDataGenerator = new _time_panel_data_generator.TimePanelDataGenerator(viewDataGenerator);
        ...
        this.groupedDataMapProvider = new _grouped_data_map_provider.GroupedDataMapProvider(this.viewDataGenerator, this.viewDataMap, this.completeViewDataMap, {
    ...
    getSkippedDaysCount(groupIndex, startDate, endDate, daysCount) {
        ...
        const includedDays = groupedData.reduce((rowCount, row) => rowCount + row.filter(_ref2 => {
            let {
                cellData: cellData
            } = _ref2;
            return startDate.getTime() < cellData.endDate.getTime() && endDate.getTime() > cellData.startDate.getTime()
        }).length, 0);
    ...
    adjustEndDateByDaylightDiff(startDate, endDate) {
        const daylightDiff = _utils_time_zone.default.getDaylightOffsetInMs(startDate, endDate);
    ...
    getCellsBetweenHorizontalView(cellMatrix, firstIdx, lastIdx) {
        return cellMatrix.reduce((result, row) => result.concat(row.filter(_ref3 => {
            let {
                index: index
            } = _ref3;
