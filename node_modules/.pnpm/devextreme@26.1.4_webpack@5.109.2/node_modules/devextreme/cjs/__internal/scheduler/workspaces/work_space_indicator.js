/**
 * DevExtreme (cjs/__internal/scheduler/workspaces/work_space_indicator.js)
 * Version: 26.1.4
...
class SchedulerWorkSpaceIndicator extends _work_space.default {
    getToday() {
        const {
            viewOffset: viewOffset,
            indicatorTime: indicatorTime
        } = this.option();
        const today = (0, _index.getToday)(indicatorTime, this.timeZoneCalculator);
    ...
    getRtlOffset(width) {
        return this.option("rtlEnabled") ? (0, _position.getBoundingRect)(this.$dateTableScrollable.$content().get(0)).width - this.getTimePanelWidth() - width : 0
    }
    ...
    getIndicatorDaysSpan() {
        ...
        if ((this.option().skippedDays ?? []).length > 0) {
            const skippedDaysDuration = this.getSkippedDaysCount(this.getStartViewDate(), Math.round(timeDiff / toMs("day"))) * toMs("day");
            timeDiff -= skippedDaysDuration
        }
        return Math.ceil((timeDiff + 1) / toMs("day"))
    }
    getIndicationHeight() {
        const today = _utils_time_zone.default.getDateWithoutTimezoneChange(this.getToday());
    ...
    getCurrentTimePanelCellIndices() {
        ...
        let cellIndices = [currentTimeRowIndex];
        ...
        const verticalGroupCount = this.isVerticalGroupedWorkSpace() ? this.getGroupCount() : 1;
    ...
    renderDateTimeIndication() {
        ...
        if (this.option("shadeUntilCurrentTime")) {
            this.shader.render(this.isHorizontalGroupedWorkSpace(), this.getGroupCount() || 1, this.getCellCount())
        }
