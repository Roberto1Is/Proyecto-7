...
function registerPalette(name, palette) {
    ...
    if (Array.isArray(palette)) {
        ...
        item.simpleSet = Array.isArray(palette.simpleSet) ? palette.simpleSet.slice(0) : void 0;
        item.indicatingSet = Array.isArray(palette.indicatingSet) ? palette.indicatingSet.slice(0) : void 0;
        item.gradientSet = Array.isArray(palette.gradientSet) ? palette.gradientSet.slice(0) : void 0;
        ...
        getColor(index, count) {
            ...
            if (cycles > 1) {
                return function(color, cycleIndex, cycleCount) {
                    const hsl = new _color.Color(color).hsl;
                    let l = hsl.l / 100;
                    ...
                    hsl.l = 100 * l;
                    return _color.Color.fromHSL(hsl).toHex()
            ...
            for (let i = 0; i < paletteLength; i++) {
                ...
                if (!color) {
                    ...
                    if (!color1) {
                        ...
                        const color2 = new _color.Color(c2[0]);
...
function getNewColor(currentColor, step) {
    let newColor = new _color.Color(currentColor).alter(step);
    const lightness = getLightness(newColor);
    if (lightness > 200 || lightness < 55) {
        newColor = new _color.Color(currentColor).alter(-step / 2)
    }
...
function createDiscreteColors(source, count) {
    ...
    for (i = 0; i <= sourceCount; ++i) {
        colors.push(new _color.Color(source[i]))
    }
    if (colorCount > 0) {
...
function getGradientPalette(source, themeDefaultPalette) {
    ...
    const color1 = new _color.Color(palette[0]);
    const color2 = new _color.Color(palette[1]);
    return {
