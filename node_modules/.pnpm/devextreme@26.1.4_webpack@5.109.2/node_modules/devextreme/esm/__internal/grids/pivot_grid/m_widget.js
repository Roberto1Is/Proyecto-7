...
import {
    FAKE_TABLE_CLASS,
    HORIZONTAL_HEADERS_AREA_CLASS,
    VERTICAL_HEADERS_AREA_CLASS
} from "./keyboard_navigation/const";
import {
    RovingTabIndex
} from "./keyboard_navigation/roving_tab_index";
import {
    getAdjacentCell
} from "./keyboard_navigation/table_cell_navigation";
import {
    findField,
...
class PivotGrid extends Widget {
    ...
    _getCellAreaNavigation(cell) {
        ...
        if (null !== (_cell$closest = cell.closest) && void 0 !== _cell$closest && _cell$closest.call(cell, `.${FAKE_TABLE_CLASS}`)) {
            return
        }
        if (null !== (_cell$closest2 = cell.closest) && void 0 !== _cell$closest2 && _cell$closest2.call(cell, `thead.${HORIZONTAL_HEADERS_AREA_CLASS}`)) {
            return this._getColumnsAreaNavigation()
        }
        if (null !== (_cell$closest3 = cell.closest) && void 0 !== _cell$closest3 && _cell$closest3.call(cell, `tbody.${VERTICAL_HEADERS_AREA_CLASS}`)) {
    ...
    _renderDescriptionArea() {
        ...
        let $targetContainer;
        let buttonStylingMode = isGeneric(current()) ? "contained" : "text";
        ...
        if (this.option("fieldChooser.enabled")) {
            this.$fieldChooserButton = $(DIV).appendTo($toolbarContainer).addClass("dx-pivotgrid-field-chooser-button");
            const buttonOptions = {
                icon: "columnchooser",
    ...
    _createTableElement() {
        const $table = $("<table>").attr("role", "presentation").css({
            width: "100%"
        ...
        eventsEngine.on($table, addNamespace("keydown", "dxPivotGrid"), "td", this._handleCellKeyDown.bind(this));
        eventsEngine.on($table, addNamespace("focusin", "dxPivotGrid"), "td", this._handleCellFocusIn.bind(this));
        return $table
    }
    ...
    _initMarkup() {
        super._initMarkup();
        this.$element().addClass("dx-pivotgrid").attr("role", "group").attr("aria-label", localizationMessage.format("dxPivotGrid-ariaLabel"))
    }
    _renderContentImpl() {
        const that = this;
    ...
    _setAriaGridAttributes(dataArea, rowsArea, columnsArea) {
        ...
        const tableIds = [columnsArea.tableElement().attr("id"), rowsArea.tableElement().attr("id"), dataArea.tableElement().attr("id")].filter(isDefined).join(" ");
