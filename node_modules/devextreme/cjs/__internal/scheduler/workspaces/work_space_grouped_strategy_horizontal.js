/**
 * DevExtreme (cjs/__internal/scheduler/workspaces/work_space_grouped_strategy_horizontal.js)
 * Version: 26.1.4
...
class HorizontalGroupedStrategy {
    ...
    getWorkSpaceMinWidth() {
        const workSpaceElementWidth = (0, _position.getBoundingRect)(this.config.getElement()).width;
        return workSpaceElementWidth - this.config.getTimePanelWidth() - 2 * _const.WORK_SPACE_BORDER_PX
    }
    getAllDayOffset() {
    ...
    getShaderOffset(i, width) {
        ...
        if (this.config.rtlEnabled()) {
            const containerWidth = (0, _position.getBoundingRect)(this.config.getScrollableContentElement()).width;
    ...
    getShaderMaxHeight(groupIndex) {
        return (0, _position.getBoundingRect)(this.config.getScrollableContentElement()).height
    }
    getShaderWidth() {
    ...
    addLastGroupCellClass(cellClass, index, applyUnconditionally) {
        if (applyUnconditionally) {
            return `${cellClass} ${_classes.LAST_GROUP_CELL_CLASS}`
        }
    ...
    addFirstGroupCellClass(cellClass, index, applyUnconditionally) {
        if (applyUnconditionally) {
            return `${cellClass} ${_classes.FIRST_GROUP_CELL_CLASS}`
        }
        const groupByDate = this.config.isGroupedByDate();
        if (groupByDate) {
