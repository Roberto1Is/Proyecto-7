...
_search_box_controller.default.setEditorClass(_text_box.default);
class DataAdapter {
    constructor(options) {
        this.options = {
            dataAccessors: {},
    ...
    setOption(name, value) {
        ...
        if ("recursiveSelection" === name || "disabledNodeSelectionMode" === name) {
    ...
    _isNodeVisible(node) {
        return false !== (null === node || void 0 === node ? void 0 : node.internalFields.item.visible)
    }
    _isNodeDisabled(node) {
        return (null === node || void 0 === node ? void 0 : node.internalFields.disabled) ?? false
    }
    _getByKey(data, key) {
        ...
        (0, _iterator.each)(this._dataStructure, (_index, node) => {
            ...
            if (parent && node.internalFields.parentKey !== this.options.rootValue) {
                this._iterateParents(node, parentNode => {
                    if ("never" === this.options.disabledNodeSelectionMode && this._isNodeDisabled(parentNode)) {
                        return
                    }
    ...
    _iterateChildren(node, recursive, callback, processedKeys) {
        ...
        if ("never" === this.options.disabledNodeSelectionMode && this._isNodeDisabled(node)) {
            return
        }
    ...
    _calculateSelectedState(node) {
        ...
        let result = false;
        const isSkipDisabled = "never" === this.options.disabledNodeSelectionMode;
        ...
        if (selectedItemsCount) {
            result = selectedItemsCount === itemsCount - subtractedItemsCount ? true : void 0
        }
        return result
    }
    _toggleChildrenSelection(node, state) {
        this._iterateChildren(node, true, child => {
            ...
            if ("recursiveAndAll" === this.options.disabledNodeSelectionMode || !this._isNodeDisabled(child)) {
    ...
    toggleSelectAll(state) {
        ...
        (0, _iterator.each)(dataStructure, (_index, node) => {
            ...
            if ("recursiveAndAll" === this.options.disabledNodeSelectionMode || !this._isNodeDisabled(node)) {
                this._setFieldState(node, SELECTED, state)
            }
        });
        this._selectedNodesKeys = this._updateNodesKeysArray(SELECTED);
    ...
    isAllSelected() {
        const selectedDisabledNodesAmount = (0, _m_array.getIntersection)(this.getSelectedNodesKeys(), this.getDisabledNodesKeys()).length;
        const isSkipDisabled = "never" === this.options.disabledNodeSelectionMode;
        const selectedNodesKeysAmount = this.getSelectedNodesKeys().length - (isSkipDisabled ? selectedDisabledNodesAmount : 0);
        if (!selectedNodesKeysAmount) {
            return false
        }
        const subtractedNodesAmount = this.getVisibleItemsCount() - (isSkipDisabled ? this._getDisabledItemsCount() : 0);
        return selectedNodesKeysAmount === subtractedNodesAmount ? true : void 0
    }
