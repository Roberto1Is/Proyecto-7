...
export class SearchPanelViewController extends modules.ViewController {
    ...
    getSearchPanelToolbarItem() {
        ...
        return {
            template: (_data, _index, container) => {
                if (this.headerPanel) {
                    const $search = $("<div>").addClass(this.headerPanel.addWidgetPrefix("search-panel")).appendTo(container);
    ...
    getSearchTextEditor() {
        ...
        const $searchPanel = $element.find(`.${this.headerPanel.addWidgetPrefix("search-panel")}`).filter((_, el) => $(el).closest(`.${headerPanelClass}`).is($element));
